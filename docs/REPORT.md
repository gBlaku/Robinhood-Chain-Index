...
roughly 11 s/block rather than 100 ms, because Nitro only produces blocks on demand and
...
public launch date in the brief. Real public day one is blocks 653,325 to 954,800,
not 0 to 864,000. Chronology below is anchored to observed timestamps, not to the
...
keeping only 3-topic logs with a ≥32-byte data payload. That excludes ERC-721,
...
> |---|---|---|---|---|---|
> | **56** | **2026-05-05 17:59:49** | **USDG** | **Global Dollar** | `0x68184c449e1a8f34fa18d289737129fd27b66f8f` | `totalSupply() == 0`, deployed but never minted |
>
...
### Classification totals, blocks 0 to 2,000,000
...
one (blocks 653,325 to 954,800), **3,859** over the following ~3 days.
...
tokens like `BEGGAR` (601 holders, 35,523 transfers), `TAXISTHEFT` (241 / 12,592),
...
No token in this set qualifies as **`no public trace`**, because Blockscout indexes
...
2. **Supply created without a `Transfer` event**: SMK2/3/4 each hold 1e24 supply
   ...
   **WETH** (block 9,482) and the first Uniswap V2 pairs, so these are infrastructure test
   tokens, not launches.
...
**Status: Pass B is complete over its intended scope**, blocks **0 to 65,000**,
...
Pass B was scoped to blocks 0 to 65,000 rather than the full pre-launch era
...
- **`Official / RWA` merges two things** the brief listed separately: Robinhood
  ...
  Paxos). Both are "not an independent launch" on the same evidence, the low-deployer
  pre-launch window, but they are not the same kind of thing.
